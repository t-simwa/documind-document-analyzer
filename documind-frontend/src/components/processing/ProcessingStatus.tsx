...
export const ProcessingStatus = ({
  ...
  return (
    <div className="w-full max-w-3xl mx-auto animate-in">
      {/* Main Card */}
      <div className="bg-white dark:bg-[#171717] border border-[#e5e5e5] dark:border-[#262626] rounded-lg shadow-none">
        {/* Header */}
        <div className="px-4 py-3 border-b border-[#e5e5e5] dark:border-[#262626]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-md bg-[#f5f5f5] dark:bg-[#262626] flex items-center justify-center flex-shrink-0">
              <ProcessingSpinnerIcon className="h-4 w-4 text-[#171717] dark:text-[#fafafa]" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-xs font-medium text-[#171717] dark:text-[#fafafa] leading-tight">
                Processing Document
              </h3>
              <p className="text-[10px] text-[#737373] dark:text-[#a3a3a3] truncate mt-0.5">
                {documentName}
              </p>
            </div>
            <div className="text-right flex-shrink-0">
              <div className="text-base font-medium text-[#171717] dark:text-[#fafafa] leading-none">
                {progressPercentage}%
              </div>
            </div>
          </div>
          ...
          {processingError && (
            <div className="p-2 rounded-md border border-red-200 dark:border-red-900/30 bg-red-50/50 dark:bg-red-950/20">
              <div className="flex items-start gap-1.5">
                <div className="w-4 h-4 rounded-md bg-red-100 dark:bg-red-900/30 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <ErrorIcon className="h-2.5 w-2.5 text-red-600 dark:text-red-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-medium text-red-900 dark:text-red-100 mb-0.5">
                    Processing Error
                  </p>
                  <p className="text-[9px] text-red-700 dark:text-red-300/80 mb-1">
                    {processingError.message}
                  </p>
                  {processingError.recoverable && onRetry && (
                    <button
                      onClick={onRetry}
                      className="inline-flex items-center gap-1 px-2 py-0.5 text-[9px] font-medium text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/30 hover:bg-red-200 dark:hover:bg-red-900/50 rounded-md transition-colors"
                    >
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div
                  key={step.id}
                  className={cn(
                    "group relative p-1.5 rounded-md border transition-all duration-200",
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <div className={cn(
                      ...
                      {isCompleted ? (
                        <CheckCompleteIcon className="h-3 w-3" />
                      ) : isActive ? (
                        <ProcessingSpinnerIcon className="h-3 w-3" />
                      ) : hasError ? (
                      ...
                      {showOCRProgress && ocrStatus && (
                        <div className="mt-1 space-y-0.5">
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-[9px] font-medium text-[#171717] dark:text-[#fafafa]">
                              {ocrStatus.progress}%
                            </span>
                          </div>
                          <div className="h-0.5 rounded-full bg-[#e5e5e5] dark:bg-[#262626] overflow-hidden">
                            <div
