"""
Microsoft SharePoint OAuth and file operations service
"""
...
class SharePointService(CloudStorageProvider):
    """Microsoft SharePoint cloud storage provider"""
    
    AUTH_BASE_URL = "https://login.microsoftonline.com/{tenant_id}/oauth2/v2.0/authorize"
    ...
    def __init__(self, access_token: str, refresh_token: Optional[str] = None, tenant_id: Optional[str] = None, site_id: Optional[str] = None):
        super().__init__(access_token, refresh_token)
        self.client_id = settings.ONEDRIVE_CLIENT_ID
        self.client_secret = settings.ONEDRIVE_CLIENT_SECRET
        self.tenant_id = tenant_id or settings.SHAREPOINT_TENANT_ID or "common"
        self.site_id = site_id  # SharePoint site ID
    ...
    async def get_authorization_url(self, redirect_uri: str, state: str) -> str:
        ...
        auth_url = self.AUTH_BASE_URL.format(tenant_id=self.tenant_id)
    ...
    async def list_files(self, folder_id: Optional[str] = None, page_token: Optional[str] = None) -> tuple[List[CloudFile], Optional[str]]:
        ...
        url = f"{self.API_BASE_URL}/sites/{self.site_id}/drive/items/{folder_id}/children"
        params = {"$top": 100}
    ...
    async def get_file(self, file_id: str) -> CloudFile:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.API_BASE_URL}/sites/{self.site_id}/drive/items/{file_id}",
                headers={"Authorization": f"Bearer {self.access_token}"}
            )
            response.raise_for_status()
            item = response.json()
    ...
    async def download_file(self, file_id: str) -> bytes:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{self.API_BASE_URL}/sites/{self.site_id}/drive/items/{file_id}/content",
                headers={"Authorization": f"Bearer {self.access_token}"}
            )
            response.raise_for_status()
    ...
    async def search_files(self, query: str, folder_id: Optional[str] = None) -> List[CloudFile]:
        ...
        search_url = f"{self.API_BASE_URL}/sites/{self.site_id}/drive/root/search(q='{query}')"
